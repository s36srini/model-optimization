...
# ==============================================================================
"""Convolution with folded batch normalization layout transform."""
...
class ConvWithFoldedBatchNorm(Conv2D):
  """Layout transformation for emulating the folding of batch normalization into Conv during serving.
  ...
  def __init__(
      ...
      name=None,
      **kwargs):
    super(ConvWithFoldedBatchNorm, self).__init__(
        filters,
        kernel_size,
        strides=strides,
        padding=padding,
        ...
        # redundant with BatchNormalization's bias
        use_bias=False,
  ...
  def build(self, input_shape):
    ...
    super(ConvWithFoldedBatchNorm, self).build(input_shape)

    conv_output_shape = self.compute_output_shape(input_shape)

    # resposible for trainable gamma and beta weights
    self.batchnorm.build(conv_output_shape)

  # Implements figure C.7 from https://arxiv.org/pdf/1712.05877.pdf.
  def call(self, inputs, training=None):
    if training is None:
      training = K.learning_phase()

    conv_out = super(ConvWithFoldedBatchNorm, self).call(inputs)
